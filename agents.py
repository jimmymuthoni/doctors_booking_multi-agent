...
#class to implent which route the workers should take
class Router(TypedDict):
    next: Literal["information_node", "booking_node", "FINISH"]
    reasoning: str

#class to define the state of the agents
...
class DoctorBookingAgent:
    ...
    def supervisor_node(self, state:AgentState)-> Command[Literal["information_node", "booking_node", "__end__"]]:
        ...
        if query:
            return Command(goto=goto, update={
                ...
                'messages': [HumanMessage(content=f"User's indentification number is {state['id_number']}")]
            })
        return Command(goto=goto, update={
            'next': goto,
            'current_reasoning': response['reasoning']
    ...
    def information_node(self, state:AgentState) -> Command[Literal['supervisor']]:
        ...
        information_agent = create_react_agent(model=self.llm_model, tool=[check_availability_by_doctor,check_availability_by_specialization] ,prompt=system_prompt)
    ...
    def workflow(self):
        ...
        self.graph.add_node(START, 'supervisor')
